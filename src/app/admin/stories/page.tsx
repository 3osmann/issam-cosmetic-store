...
const emptyItem: FormItem = { type: "image", content: "", caption: "" }

const emptyForm: FormState = { items: [{ ...emptyItem }], duration: 1, link: "", active: true }
...
export default function AdminStoriesPage() {
  ...
  function openEdit(item: Story) {
    const formItems: FormItem[] = (item.items && item.items.length > 0)
      ? item.items.map((i) => ({ type: i.type, content: i.content, caption: i.caption || "" }))
      : [{ type: item.type, content: item.content, caption: item.caption || "" }]
    setEditing(item)
    setForm({ items: formItems, duration: item.duration, link: item.link || "", active: item.active })
    setShowForm(true)
    setFormError("")
  }
  ...
  async function handleVideoFile(index: number, e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      updateItem(index, { content: dataUrl, type: "video" })
    } catch (err) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const validItems = form.items.filter((item) => item.content)
    if (!validItems.length) { setFormError("At least one item with content is required"); return }
    try {
      setSubmitting(true)
      setFormError("")
      let res
      const body = JSON.stringify({ items: validItems, duration: form.duration, link: form.link || null, active: form.active })
      if (editing) {
        ...
        <div className="admin-card" style={{ marginBottom: 24 }}>
          ...
          <div className="admin-card-body">
            <form onSubmit={handleSubmit}>
              ...
              <div style={{ display: "flex", gap: 16, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
                <div className="admin-form-group" style={{ marginBottom: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    {[1, 2, 3].map((d) => (
                      <button key={d} type="button" className={`admin-filter-btn ${form.duration === d ? "active" : ""}`} onClick={() => setForm({ ...form, duration: d })}>
                        {d} Day{d > 1 ? "s" : ""}
                      </button>
                    ))}
                  </div>
              ...
              <div style={{ marginBottom: 12, fontWeight: 600, fontSize: 14, color: "var(--admin-text)" }}>Story Items</div>

              {form.items.map((item, idx) => (
                <div key={idx} style={{ border: "1px solid var(--admin-border)", borderRadius: 8, padding: 12, marginBottom: 12, position: "relative" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    <strong style={{ fontSize: 13, color: "var(--admin-text-muted)" }}>Item #{idx + 1}</strong>
                    <button type="button" className="admin-btn admin-btn-ghost admin-btn-sm" onClick={() => removeItem(idx)} style={{ color: "#ef4444" }}>Remove</button>
                  </div>
                  <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                    {(["image", "text", "video"] as const).map((t) => (
                      <button key={t} type="button" className={`admin-filter-btn ${item.type === t ? "active" : ""}`} onClick={() => updateItem(idx, { type: t, content: "" })} style={{ textTransform: "capitalize" }}>
                        {t}
                      </button>
                    ))}
                  </div>
                  {item.type === "image" && (
                    <div className="admin-form-group" style={{ marginBottom: 0 }}>
                      <ImageUpload
                        value={item.content.startsWith("data:") || item.content.startsWith("/") || item.content.startsWith("http") ? item.content : ""}
                        onChange={(url) => updateItem(idx, { content: url })}
                        label="Image"
                      />
                    </div>
                  )}
                  ...
                  {item.type === "video" && (
                    <>
                      <div className="admin-form-group" style={{ marginBottom: 6 }}>
                        <input
                          className="admin-input"
                          placeholder="https://example.com/video.mp4"
                          value={item.content.startsWith("http") && !item.content.startsWith("data:") ? item.content : ""}
                          onChange={(e) => updateItem(idx, { content: e.target.value })}
                        />
                      </div>
                      <div className="admin-form-group" style={{ marginBottom: 0 }}>
                        <button type="button" className="admin-btn admin-btn-ghost admin-btn-sm" onClick={() => videoInputRefs.current[`video-${idx}`]?.click()}>
                          Upload Video File
                        </button>
                        <input ref={(el) => { videoInputRefs.current[`video-${idx}`] = el }} type="file" accept="video/*" onChange={(e) => handleVideoFile(idx, e)} style={{ display: "none" }} />
                      </div>
                    </>
                  )}
                  {item.type !== "text" && (
                    <div className="admin-form-group" style={{ marginBottom: 0, marginTop: 6 }}>
                      <input
                        className="admin-input"
              ...
              <button type="button" className="admin-btn admin-btn-ghost admin-btn-sm" onClick={addItem} style={{ marginBottom: 16 }}>
                + Add Item
              </button>
          ...
          <table className="admin-table">
            ...
            <tbody>
              {loading ? (
                ...
                paginated.map((story) => {
                  const expired = story.expired || new Date(story.expiresAt) < new Date()
                  const firstItem = story.items?.[0] || story
                  return (
                    <tr key={story.id}>
                      <td>
                        {firstItem.type === "image" ? (
                          <img src={firstItem.content} alt="Story" style={{ width: 48, height: 48, borderRadius: "50%", objectFit: "cover" }} />
                        ...
                        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                          {(story.items || []).map((item, i) => (
                            <span key={i} style={{
                              ...
                              background: item.type === "image" ? "#dbeafe" : item.type === "video" ? "#fce7f3" : "#fef3c7",
                              color: item.type === "image" ? "#1d4ed8" : item.type === "video" ? "#be185d" : "#b45309",
                            }}>
                              {item.type}
                            </span>
                          ))}
                      ...
                      <td>{story.duration} day{story.duration > 1 ? "s" : ""}</td>
                      <td style={{ color: "var(--admin-text-muted)", fontSize: 13 }}>
                        {new Date(story.expiresAt).toLocaleDateString()} {new Date(story.expiresAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                      </td>
                      <td>
                        {expired ? (
                          <span className="admin-badge danger">Expired</span>
                        ) : (
                          <span className={`admin-badge ${story.active ? "success" : "warning"}`}>
                            {story.active ? "Active" : "Inactive"}
                          </span>
                        )}
                      </td>
                      <td style={{ textAlign: "right" }}>
                        <button className="admin-btn-icon" onClick={() => openEdit(story)} title="Edit">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                        </button>
                        {!story.active && !expired && (
                          <button className="admin-btn-icon" onClick={() => handleReactivate(story.id)} title="Reactivate">
                        ...
                        {expired && (
                          <button className="admin-btn-icon" onClick={() => handleRepost(story.id)} title="Repost (renew)">
                        ...
                        <button className="admin-btn-icon" onClick={() => handleDeactivate(story.id)} title="Deactivate">
